// MSpin_c* torsion calculator, version 1, by Hassan Abdallah.
// This Magma program calculates an F_2-linear basis
//  for the ideal of 2-torsion in the spinc^c-cobordism ring mod 2, in a range of degrees, using element names 
//  which make it possible  to see the multiplicative structure. 
//
// For explanation of what this code does, why it works,
//  and results, see the paper "Products in spinc^c 
...
for i in [1..# MonomialsOfWeightedDegree(bso,deg)] do 
	...
	val_q0 := Evaluate(Qi(1,m_deg[i]),bspinc_gens);
	if val_q ne 0 and val_q0 eq 0 then
		Append(~b_deg,val_r);
		Append(~D, val_r);
	end if; 
